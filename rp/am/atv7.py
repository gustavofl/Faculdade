...
def ler_arquivo_pgm(arquivo):
	arq = open(arquivo, "rb")

	tipo_pgm = arq.readline() # P5

	largura,altura = arq.readline().split()
	largura = int(largura)
	altura = int(altura)

	tipo_pgm = arq.readline() # 255
	
	imagem = []
	for i in range(altura):
		for j in range(largura):
			imagem.append(ord(arq.read(1)))

	return imagem

def obter_dados_faces():
	dados = []

	for indice_pessoa in range(40):
		for indice_imagem in range(10):
			nome_arquivo = "orl_faces/s"+str(indice_pessoa+1)+"/"+str(indice_imagem+1)+".pgm"
			dados.append([ler_arquivo_pgm(nome_arquivo), indice_pessoa, indice_pessoa*10+indice_imagem])
...
def main():
	...
	print("intervalo 1-NN=",intervalo_1nn)
	print("intervalo 3-NN=",intervalo_3nn)
